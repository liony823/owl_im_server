...
import (
	...
	"github.com/openimsdk/open-im-server/v3/pkg/protocol/constant"
...
func (f *friendDatabase) AgreeFriendRequest(ctx context.Context, friendRequest *model.FriendRequest) (err error) {
	return f.tx.Transaction(ctx, func(ctx context.Context) error {
		...
		if len(adds) > 0 {
			...
			if err := f.friendRelation.Create(ctx, []*model.FriendRelation{
				{OwnerUserID: friendRequest.ToUserID, RelatedUserID: friendRequest.FromUserID},
				{OwnerUserID: friendRequest.FromUserID, RelatedUserID: friendRequest.ToUserID},
			}); err != nil {
				return err
			}
		}
...
func (f *friendDatabase) Delete(ctx context.Context, ownerUserID string, friendUserIDs []string) (err error) {
	return f.tx.Transaction(ctx, func(ctx context.Context) error {
		...
		for _, friendUserID := range friendUserIDs {
			if err := f.friendRelation.Delete(ctx, []*model.FriendRelation{
				{OwnerUserID: ownerUserID, RelatedUserID: friendUserID},
			}); err != nil {
				return err
			}
		}
